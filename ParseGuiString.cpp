...
void ParseGuiString::parseStringsInTr(const char *filename)
{
    ...
    trStringList.erase(unique(trStringList.begin(), trStringList.end()),
                       trStringList.end());
...
*/
void ParseGuiString::modifyFontRange(unsigned short start, unsigned short end,
                                     int status)
{
    int range = end - start;
    switch(status)
