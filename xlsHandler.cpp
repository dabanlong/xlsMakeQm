...
void XlsHandler::generateTSFile()
{
        ...
        else if(tt > 0)
        {
            if(pWS->rows.row[1].cells.cell[tt].str)//check the language code
            {
                ...
                ofs << "<?xml version=\"1.0\" encoding=\"utf-8\"?>" << endl
                    ...
                    <<pWS->rows.row[1].cells.cell[tt].str<<"\">"<<endl
                    << "<context>"<<endl
    ...
    for(int i = 0; i < pWB->sheets.count; ++i)
    {
        QFile fileo(QString("./ts_output/%1_%2.ts").arg("i80").arg(
                        QString::fromStdString(LANGUAGE_STRINGS[i])));
        ...
        streamo << "<?xml version=\"1.0\" encoding=\"utf-8\"?>" << endl
                << QString("<TS version=\"2.0\" language=\"%1\"><context>").arg(
                    QString::fromStdString(LANGUAGE_STRINGS[i])) << endl
                << "<name>QObject</name>" << endl;
        pWS = xls_getWorkSheet(pWB, i);
        xls_parseWorkSheet(pWS);
    ...
    system("/opt/Qt_4.8.7/bin/lrelease ./ts_output/*.ts");
#endif
}
